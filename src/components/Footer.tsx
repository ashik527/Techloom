    ...
    <footer className="bg-gray-50 border-t border-gray-200 pt-16 pb-8 px-4">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
        ...
        <div className="col-span-1 md:col-span-1">
          <Link to="/" className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xl font-bold text-gray-900">PlaxoMart</span>
          </Link>
          ...
          <div className="flex items-center gap-4">
            {settings?.facebookPage && (
              <a href={settings.facebookPage} target="_blank" rel="noopener noreferrer" className="p-2 bg-white rounded-lg border border-gray-200 text-gray-600 hover:text-indigo-600 hover:border-indigo-600 transition-all">
        ...
        <div>
          ...
          <ul className="space-y-4">
            ...
            <li><Link to="/admin" className="text-gray-600 hover:text-indigo-600 text-sm transition-colors">Admin Panel</Link></li>
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-4">
            ...
            <li><Link to="/products?category=Gaming" className="text-gray-600 hover:text-indigo-600 text-sm transition-colors">Gaming</Link></li>
            <li><Link to="/products?category=Software" className="text-gray-600 hover:text-indigo-600 text-sm transition-colors">Software</Link></li>
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-4">
            <li className="flex items-start gap-3">
              ...
              <span className="text-gray-600 text-sm">{settings?.whatsappNumber || '+880 1700-000000'}</span>
            </li>
            <li className="flex items-start gap-3">
              <Mail className="w-5 h-5 text-indigo-600 shrink-0" />
              <span className="text-gray-600 text-sm">{settings?.email || 'support@plaxomart.com'}</span>
            </li>
            <li className="flex items-start gap-3">
              <MapPin className="w-5 h-5 text-indigo-600 shrink-0" />
              <span className="text-gray-600 text-sm">{settings?.location || 'Dhaka, Bangladesh'}</span>
            </li>
          </ul>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto pt-8 border-t border-gray-200 text-center">
        <p className="text-gray-500 text-sm">
          &copy; {new Date().getFullYear()} PlaxoMart. All rights reserved.
        </p>
      </div>
    </footer>
  );
